...
# config for web VM
	config.vm.define "web", primary: true do |web|
		...
		web.vm.provision "shell", inline: "echo This is WEB"
	...
	config.vm.define "app", primary: true do |app|
		...
		app.vm.network "forwarded_port", guest: 80, host: 8082
                app.vm.provider	"virtualbox" do	|machine|
                        machine.name = "app-backend"
			machine.customize ["modifyvm", :id, "--cpuexecutioncap", "35"]
                        machine.memory = 1024
                                             	end
                app.vm.provision "shell", inline: "echo This is APP"
		app.vm.provision "shell", path: "app.sh"
                end

config.vm.provision "shell", inline: "echo Hello, World!"

config.vm.synced_folder "sources/", "/srv/website",
